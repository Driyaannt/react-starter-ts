...
const ROUTES_OUTPUT = path.join(__dirname, '../src/routes/generated-routes.tsx');
...
function generateRouteFile(adminRoutes, userRoutes) {
  const allImports = [
    ...adminRoutes.imports.map(imp => `import ${imp.name} from "${imp.path}";`),
    ...userRoutes.imports.map(imp => `import ${imp.name} from "${imp.path}";`)
  ];
